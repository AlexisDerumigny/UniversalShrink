...
```{r}
list_GMV = list(
  GMV_MP_shrinkage = GMV_Moore_Penrose_target(Y = t(X), centeredCov = TRUE),
  ...
  GMV_plugin_ridge_target_shrinked_toIP = GMV_PlugIn(
    ridge_target(Y = t(X), centeredCov = TRUE)),
  
  GMV_plugin_optimal_ridge_shrinkage = GMV_PlugIn(
    ridge_shrinkage_rescaled_optimal(Y = t(X))),
  
  GMV_plugin_inv_cov_quadratic_inverse_shrinkage = GMV_PlugIn(
    solve(cov_quadratic_inverse_shrinkage(X = X, centeredCov = TRUE))),
  
  GMV_plugin_higher_order_shrinkage_2 = GMV_PlugIn(
    ridge_higher_order_shrinkage(Y = t(X), m = 2, centeredCov = TRUE))
