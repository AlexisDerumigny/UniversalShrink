...
LossFunction_GMV <- function(weights){
  if (inherits(weights, "EstimatedPortfolioWeights")){
...
```{r}
list_GMV = list(
  GMV_MP_shrinkage = GMV_Moore_Penrose_shrinkage(X, centeredCov = TRUE),
  
  GMV_MP = GMV_Moore_Penrose(X, centeredCov = TRUE),
  
  GMV_plugin_MP_shrinked_toIP = GMV_PlugIn(
    Moore_Penrose_shrinkage(X, centeredCov = TRUE)),
  
  GMV_plugin_ridge_shrinkage_shrinked_toIP = GMV_PlugIn(
    ridge_shrinkage(X, centeredCov = TRUE)),
  
  GMV_plugin_optimal_ridge_shrinkage = GMV_PlugIn(
    ridge_shrinkage_Wang(X)),
  
  GMV_plugin_inv_cov_quadratic_inverse_shrinkage = GMV_PlugIn(
    solve(cov_quadratic_inverse_shrinkage(X, centeredCov = TRUE))),
  
  GMV_plugin_higher_order_shrinkage_2 = GMV_PlugIn(
    ridge_higher_order_shrinkage(X, m = 2, centeredCov = TRUE))
  ...
  #   ridge_higher_order_shrinkage_optimal(X, m = 3, centeredCov = TRUE)),
  
  # GMV_plugin_higher_order_shrinkage_4 = GMV_PlugIn(
  #   ridge_higher_order_shrinkage_optimal(X, m = 4, centeredCov = TRUE))
  # ,
  
  # GMV_plugin_higher_order_shrinkage_5 = GMV_PlugIn(
  #   ridge_higher_order_shrinkage_optimal(X, m = 5, centeredCov = TRUE)),
  
  # GMV_plugin_higher_order_shrinkage_6 = GMV_PlugIn(
  #   ridge_higher_order_shrinkage_optimal(X, m = 6, centeredCov = TRUE))
)
```
